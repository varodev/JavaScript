...
function comprobacionContenidoH1(arrayContainers, arrayNum, index) {
  ...
  if (textoH1 == null) {
    let num_select_array = Math.floor(Math.random() * 2);
    creacionDeContenido(container, arrayNum[index],num_select_array);
  } else {
...
// Parametros =
function creacionDeContenido(container, index,num_select_array) {
  ...
  if(num_select_array == 0){
    ...
    imgCreada.src = "images/dado_" +(index+1) + ".jpg";
    imgCreada.setAttribute("alt", "Imagen");
    imgCreada.setAttribute("id", "img_" + index);
    container.appendChild(imgCreada);
  }
}
...
function mostrarDado() {
  ...
  // Compruebo si hay alguna imagen creada en el lugar del dado
  let container_derecha = document.getElementById('derecha');
  let img_dado_creada = container_derecha.getElementsByTagName("img")[0];
...
function mostrarCasilla() {
  ...
  let numero = container_derecha.getElementsByTagName("img")[0];
...
function seleccionarCasillaNueva(casilla, casillaInicial) {
  ...
  //Compruebo si hay una img en la casilla
  let imagen_casilla = casilla.getElementsByTagName('img')[0];
  let valorCasilla;

  if(imagen_casilla == null || imagen_casilla == undefined){
    ...
    valorCasilla = casillaNumero.innerHTML;
  } else {
    imagen_casilla.style.opacity = 1;
    valorCasilla = 10;
  }
